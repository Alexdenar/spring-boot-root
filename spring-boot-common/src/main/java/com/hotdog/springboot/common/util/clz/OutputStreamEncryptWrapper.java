 ...
 * 
 */
package com.hotdog.springboot.common.util.clz;

import com.hotdog.springboot.common.util.Constants;
import com.hotdog.springboot.common.util.encryptionAndDecryption.EncryptUtil;
...
public class OutputStreamEncryptWrapper extends OutputStream {
	...
	private void writeEncrypt() throws IOException{
		String rspMsgPlain = new String(buffer.toByteArray(), Constants.CHARSET_NAME);
		JSONObject reqJson = JSONObject.fromObject(rspMsgPlain);
		String rspMsg = EncryptUtil.encryptMsg(reqJson, secretKey);
		byte[] bytes = rspMsg.getBytes(Constants.CHARSET_NAME);
